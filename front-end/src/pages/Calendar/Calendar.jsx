...
import { api, googleAPI } from '~/main'
...
const Calendar = ({ userId, lichType }) => {
  ...
  const fileName = `${userId}-${lichType == 'ShowExam' ? '-lichThi' : '-lichHoc'}.csv`

  useEffect(() => {
    const getHtmlCalendar = async () => {
      try {
        const response = await axios.get(api + '/get-vlu-calendar', {
          params: { userId }
        })
        return response.data
      } catch (error) {
        return 'Khổng thể kết nối tới server!' + error.message
      }
    }
  ...
  const handleExportCalendar = async () => {
    const getCalendarJson = await axios.get(api + '/export-calendar-to-csv', {
      params: { userId, lichType },
      responseType: 'blob'
    })
  ...
  const handleLogin2Google = () => {
    window.open(`${googleAPI}/authenticate`, '_blank')
  }

  const handleImportCalendar2Google = async () => {
    setOnLoad(true)
    try {
      const callApi = await axios.get(googleAPI + '/calendar', {
        params: {
          userId,
          token: googleToken,
      ...
      setImportCalendarReport('Done! Hãy kiểm tra lịch trên Google Calendar nhé.')
      setOnLoad(false)
    } catch (error) {
  ...
  return (
    <Container sx={{ width: '100%', height: '100vh', py: '16px' }}>
      ...
      >
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, pt: '8px' }}>
          <Box sx={{ display: 'flex', gap: 1 }}>
            ...
            >
              <Typography variant="small" sx={{ fontSize: { md: '14px', xs: '10px' } }}>
                Quay lại
              </Typography>
            </CalendarButton>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'center' }}>
            {importCalendarReport ? <Typography sx={{ color: '#eeeeee' }}>{importCalendarReport}</Typography> : null}
          </Box>
        </Box>
